...
export class MrPackBuilder {
   ...
   */
  async scanModsDirectory (modsDir, { ensureMods = [], ignoreMods = [], ensureOptional = [], includeAll = false } = {}) {
    let modFiles = [];
    ...
    for (const modData of indexFiles) {
      ...
      if (modData.download.mode === 'url') {
        ...
        modFiles.push({
          ...
          env:       {
            client: ensureOptional.some(reg => reg.test(modData.filename)) ? 'optional' : sideCheck('client', modData.side, isDisabled),
  ...
  async buildArchive (outputPath) {
    return new Promise((resolve, reject) => {
      ...
      output.on('close', () => {
        console.log(`Pack built: ${outputPath} (${Math.round(archive.pointer() / 10000)/100} MB)`);
        resolve();
      });
