...
export function cleanDsStoreRecursively (dirPath) {
  const files = fs.readdirSync(dirPath);
  ...
  for (const file of files) {
    ...
    if (stats.isDirectory()) {
      cleanDsStoreRecursively(filePath);
...
export function ftbQuestsCheck () {
  const configDirName = join('config', 'ftbquests');
  const configDir     = resolve(join('..', configDirName));
  const defaultsDir   = resolve(join('..', 'configureddefaults', configDirName));

  if (!fs.existsSync(configDir)) {
    console.error('failed to find directory:', configDir);
    return false;
  }
  if (!fs.existsSync(defaultsDir)) {
    console.error('failed to find directory:', defaultsDir);
    return false;
  }

  cleanDsStoreRecursively(configDir);
  cleanDsStoreRecursively(defaultsDir);

  return getDirectorySize(defaultsDir) === getDirectorySize(configDir);
}

/**
 * Resizes an icon to 64x64 pixels
 * @param {string} iconPath - Path to the source icon file
 * @param {string} outputPath - Optional path to save the resized icon. If not provided, overwrites the source file.
 * @returns {Promise<string>} - Path to the resized icon
 */
export async function resizeIconTo64px (iconPath, outputPath) {
  // First, make sure the sharp package is installed
  // You can add it to your project with: npm install sharp
  ...
  try {
    ...
    console.error(`Error resizing icon ${iconPath}:`, error);
    throw error;
  }
}
